...
function startRound() {
    ...
    players.forEach((player) => { player.cashedOut = false; });
...
function crash() {
    ...
    players.forEach((player, socketId) => {
        if (player.hasBet && !player.cashedOut) {
            ...
            player.hasBet = false;
            io.to(socketId).emit('bet:lost', { amount: player.betAmount, balance: player.balance });
        }
        player.hasBet = false;
        player.betAmount = 0;
    });
    ...
    players.forEach((player, socketId) => {
        io.to(socketId).emit('player:data', { balance: player.balance });
    });
...
io.on('connection', (socket) => {
    ...
    socket.on('bet:place', (data) => {
        ...
        playerBets.push({ socketId: socket.id, amount: amount });

        socket.emit('bet:confirmed', { amount: amount, balance: player.balance });
        broadcastPlayerList();
    });
    ...
    socket.on('bet:cashout', () => {
        ...
        if (index !== -1) playerBets.splice(index, 1);
